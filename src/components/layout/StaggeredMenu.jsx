...
/**
 * StaggeredMenu
 * ใช้เป็น Hamburger Menu / Offcanvas Menu แบบมี animation
 *
 * props หลัก ๆ ที่ใช้กับ Navbar:
 * - position: "right" | "left"   // เมนูโผล่ด้านไหน
 * - colors: string[]             // สี layer พื้นหลังตอน slide
 * - items: { label, link, ariaLabel }[]
 * - socialItems: { label, link }[]
 * - displaySocials: boolean      // แสดงกล่อง Social หรือไม่
 * - displayItemNumbering: boolean// แสดงเลขลำดับเมนูด้านขวา
 * - logoUrl: string              // path โลโก้
 * - menuButtonColor: string      // สีปุ่มตอนปิด
 * - openMenuButtonColor: string  // สีปุ่มตอนเปิด
 * - changeMenuColorOnOpen: boolean
 * - isFixed: boolean             // true = ครอบเต็มหน้าจอ (ใช้ทำ full-screen overlay)
 * - accentColor: string          // สี accent (ใช้กับเลขและหัวข้อ Socials)
 */
export const StaggeredMenu = ({
  ...
  menuButtonColor = "#ffffff", // สีตอนปิด
  openMenuButtonColor = "#ffffff", // สีตอนเปิดตอนเปิด
  ...
  // ตั้งค่าตำแหน่งเริ่มต้นของ panel และ layer (อยู่นอกจอ)
  useLayoutEffect(() => {
  ...
  // สร้าง timeline เวลาเมนูเปิด (slide + stagger animation)
  const buildOpenTimeline = useCallback(() => {
    ...
    // ตัวอักษรเมนูหลักเด้งขึ้นมาทีละตัว
    if (itemEls.length) {
  ...
  // animation ตัวอักษร "Menu" / "Close" ให้หมุนเปลี่ยนหลายรอบ
  ...
  // toggle เมนู (เรียกใช้โดยปุ่ม hamburger และตอนคลิกเมนู)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Header: โลโก้ + ปุ่ม Hamburger */}
        <header
          className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between p-[2em] bg-zinc-900/80 pointer-events-none z-20"
          aria-label="Main navigation header"
        >
          <div
            aria-label="Logo"
            className={`flex text-3xl font-bold font-mono items-center select-none pointer-events-auto ${
          ...
          <button
            ref={toggleBtnRef}
            className="sm-toggle relative inline-flex items-center gap-[0.3rem] border-0 cursor-pointer text-[#e9e9ef] text-3xl  font-medium leading-none overflow-visible pointer-events-auto"
        ...
        {/* Panel ด้านขวา (รายการเมนู + Socials) */}
        <aside
          ...
          aria-hidden={!open}
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5 py-8">
            <ul
            ...
            >
              {items && items.length ? (
                items.map((it, idx) => (
                  <li
                    className="sm-panel-itemWrap relative overflow-hidden leading-none pt-2"
                    key={it.label + idx}
                  >
                    {/* ใช้ NavLink กับ React Router + คลิกแล้วปิดเมนู */}
                    <NavLink
      ...
      {/* CSS เฉพาะของเมนูนี้ (แยก namespace ด้วย .sm-scope) */}
      <style>{`
...
/* สำหรับจอเล็ก ให้ panel กว้างเต็ม */
